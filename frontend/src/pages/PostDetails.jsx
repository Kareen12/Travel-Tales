    ...
    <div>
        ...
        <div className="flex justify-between items-center pt-16">
         ...
         {user?._id===post?.userId && <div className="flex items-center justify-center space-x-2">
            <p className="cursor-pointer text-2xl" onClick={()=>navigate("/edit/"+postId)} ><BiEdit/></p>
            <p className="cursor-pointer text-red-500 text-2xl" onClick={handleDeletePost}><MdDelete/></p>
         </div>}
        </div>
        <div className="flex items-center justify-between mt-2 md:mt-4">
        <p>@{post.username}</p>
       <div className="flex space-x-2 max-[640px]:text-[13px]">
        ...
        <img src={IF+post.photo} className="w-full  mx-auto mt-8" alt=""/>
         ...
         <div className="flex items-center mt-8 space-x-4 font-semibold">
          ...
          {post.categories?.map((c,i)=>(
            <>
            <div key={i} className="bg-gray-300 rounded-lg px-3 py-1 max-[640px]:text-[13px]">{c}</div>
            </>
